...
predictor<-xreturns$capex

# Create a new data frame of predictor and current returns 
sortdat<-data.table(permno=xreturns$permno,date=xreturns$date,eret=xreturns$eret,predictor)

# Keep if not missing lagged momentum and current returns
sortdat<-sortdat[!is.na(sortdat$predictor) & !is.na(sortdat$eret),]

# Summary statistics of predictor and current returns
sumstat<-data.table(predictor=sortdat$predictor,eret=sortdat$eret)
sink("summary.txt")
...
HP<-decileports_wide[1,]-decileports_wide[10,] # returns of the long-short decile hedge portfolio over time
